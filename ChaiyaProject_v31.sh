...
async function loadOnline() {
  ...
  if (!r.connections.length) {
    el.innerHTML = `<div style="text-align:center;color:var(--muted);padding:2rem">ไม่มี connection ขณะนี้</div>`;
  } else {
  ...
  // Data usage — ดึงจาก SSH users (แยกจาก x-ui)
  ...
  if (!users.error && users.users) {
    dl.innerHTML = users.users.map(u => {
      ...
      return `<div style="margin-bottom:.8rem">
        ...
        ${limit>0?`<div class="progress"><div class="progress-bar" style="width:${pct}%"></div></div>`:''}
      </div>`;
    }).join('');
  }
}
...
# used_gb จะเป็น 0 เสมอตอนสร้างใหม่ (ดึงจริงไม่ได้ตอน gen html)
used_gb = 0
outfile = """$outfile"""
...
# คำนวณ % สำหรับ progress bar (0 เมื่อ unlimited)
bar_pct = 0
bar_label = "ไม่จำกัด"
if dg_num > 0:
    bar_pct = min(100, round(used_gb / dg_num * 100))
    bar_label = f"{used_gb} / {dg_num} GB"

# สี progress bar: เขียว→เหลือง→แดง ตาม %
def bar_color(pct):
...
@keyframes barFill{from{width:0}to{width:""" + str(bar_pct) + """%}}
...
<div class="wrap">
  ...
  <div class="data-wrap">
    <div class="data-top">
      ...
      <div class="data-txt">""" + bar_label + """</div>
    </div>""" + ("""
    <div class="bar-track">
      <div class="bar-fill" style="width:""" + str(bar_pct) + """%"></div>
    </div>
    <div class="bar-pct">""" + str(bar_pct) + """%</div>""" if dg_num > 0 else """
    <div class="bar-track">
      <div class="bar-fill" style="width:100%;background:linear-gradient(90deg,#00ff80,#00d4ff,#b400ff);animation:none"></div>
    </div>
    <div class="bar-pct" style="color:#00ff8088">\u221e \u0e44\u0e21\u0e48\u0e08\u0e33\u0e01\u0e31\u0e14</div>""") + """
  </div>
