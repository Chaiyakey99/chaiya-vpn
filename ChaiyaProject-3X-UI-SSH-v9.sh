...
HAPROXYEOF
  ...
  if haproxy -c -f /etc/haproxy/haproxy.cfg &>/dev/null; then
    ...
    systemctl restart haproxy \
      && ok "HAProxy พร้อม (80→Nginx L7, 443→Xray TLS)" \
...
# หมายเหตุ: "UDP Custom" เป็น binary เฉพาะทางที่ไม่มีต้นทางทางการ/เวอร์ชันที่ยืนยันได้ปลอดภัย
# สคริปต์นี้เตรียม service + firewall + DNAT ไว้ให้ครบ แต่ "ไม่" ดาวน์โหลด binary จากแหล่งที่ยืนยันตัวตนไม่ได้มาแทนคุณ
# วิธีใช้งาน: นำ binary ที่คุณไว้ใจไปวางที่ /usr/local/bin/udp-custom (ต้องรับ flag -config)
# แล้ว: systemctl enable --now chaiya-udp-custom
...
[Unit]
Description=Chaiya UDP Custom (:${UDPCUSTOM_PORT}) — ต้องวาง binary เองที่ /usr/local/bin/udp-custom
After=network.target
[Service]
Type=simple
...
if [[ -x /usr/local/bin/udp-custom ]]; then
  systemctl enable --now chaiya-udp-custom 2>/dev/null \
    ...
    || warn "UDP Custom start ไม่สำเร็จ"
else
...
[[ $USE_SSL -eq 0 ]] && _PANEL_URL="http://${DOMAIN}:${DASHBOARD_PORT}"
