...
# ══════════════════════════════════════════════════════════════
# เมนู 18 — SSH Dropbear + HTTP-CONNECT Manager
# ══════════════════════════════════════════════════════════════
menu_18() {
  clear
  MY_IP=$(curl -s --max-time 5 ifconfig.me 2>/dev/null || hostname -I | awk '{print $1}')
  ...
  CONNS=$(ss -tn state established 2>/dev/null | grep -cE ':80 |:143 |:109 ' || echo "0")

  _svc_color() { [[ "$1" == "active" ]] && printf "${GR}✅ RUNNING${RS}" || printf "${RD}❌ STOPPED${RS}"; }

  printf "\n${R1}╔══════════════════════════════════════════════════════╗${RS}\n"
  printf "${R1}║${RS}  🔥 ${WH}SSH Dropbear + HTTP-CONNECT Manager${RS}           ${R1}║${RS}\n"
  printf "${R1}╠══════════════════════════════════════════════════════╣${RS}\n"
  printf "${R1}║${RS}  🌐 Dashboard : ${CY}%s://%s:81/sshws/sshws.html${RS}\n" "$PROTO" "$_H"
  printf "${R1}║${RS}  🔑 Token     : ${YE}%s${RS}\n" "$TOKEN"
  printf "${R1}╠══════════════════════════════════════════════════════╣${RS}\n"
  printf "${R1}║${RS}  🚇 HTTP-CONNECT (port 80) : $(_svc_color "$WS_ST")         ${R1}║${RS}\n"
  printf "${R1}║${RS}  🐻 Dropbear (port 143/109): $(_svc_color "$DB_ST")         ${R1}║${RS}\n"
  printf "${R1}║${RS}  🎮 badvpn-udpgw (port 7300): $(_svc_color "$UDPGW_ST")    ${R1}║${RS}\n"
  printf "${R1}║${RS}  👥 Active connections: ${WH}%s${RS}                         ${R1}║${RS}\n" "$CONNS"
  printf "${R1}╠══════════════════════════════════════════════════════╣${RS}\n"
  printf "${R1}║${RS}  ${YE}📱 Config สำหรับแอพ (NetMod/HTTP Injector/KPN):${RS}  ${R1}║${RS}\n"
  printf "${R1}║${RS}  ${WH}  Host    : %s${RS}\n" "$_H"
  printf "${R1}║${RS}  ${WH}  Port    : 80 (HTTP-CONNECT tunnel)${RS}\n"
  printf "${R1}║${RS}  ${WH}  Payload : CONNECT /  HTTP/1.1[crlf]${RS}\n"
  printf "${R1}║${RS}  ${WH}            Host: [bug_host][crlf][crlf]${RS}\n"
  printf "${R1}║${RS}  ${WH}  Proxy   : ไม่ต้องใช้ proxy ภายนอก${RS}\n"
  printf "${R1}║${RS}  ${WH}  User/Pass: SSH user ที่สร้างใน dashboard${RS}\n"
  printf "${R1}║${RS}  ${WH}  UDPGW   : 127.0.0.1:7300 (สำหรับ game)${RS}\n"
  printf "${R1}╠══════════════════════════════════════════════════════╣${RS}\n"
  printf "${R1}║${RS}  ${GR}1.${RS} 🔄 Restart ทุก service              ${R1}║${RS}\n"
  printf "${R1}║${RS}  ${RD}2.${RS} ⏹  หยุด HTTP-CONNECT tunnel         ${R1}║${RS}\n"
  printf "${R1}║${RS}  ${GR}3.${RS} ▶️  เริ่ม HTTP-CONNECT tunnel         ${R1}║${RS}\n"
  printf "${R1}║${RS}  ${CY}4.${RS} 🎮 Restart badvpn-udpgw             ${R1}║${RS}\n"
  ...
  case $_sub18 in
    1)
      systemctl restart chaiya-sshws dropbear 2>/dev/null
      ...
      printf "\n${GR}✅ Restart ทุก service สำเร็จ${RS}\n"
      sleep 2; menu_18 ;;
    2)
      systemctl stop chaiya-sshws 2>/dev/null
      printf "\n${YE}⏹ หยุด HTTP-CONNECT tunnel แล้ว${RS}\n"
      sleep 2; menu_18 ;;
    3)
      systemctl start chaiya-sshws dropbear 2>/dev/null
      printf "\n${GR}▶️ เริ่ม service แล้ว${RS}\n"
      sleep 2; menu_18 ;;
    4)
      pkill -f badvpn-udpgw 2>/dev/null; sleep 1
      screen -dmS badvpn7300 badvpn-udpgw --listen-addr 127.0.0.1:7300 --max-clients 500 2>/dev/null
