...
<script>
const API='';
const TOKEN='%%BAKED_TOKEN%%'||new URLSearchParams(location.search).get('token')||document.cookie.match(/token=([^;]+)/)?.[1]||'';
...
async function api(method,path,body=null){const opts={method,headers:{'Content-Type':'application/json','Authorization':'Bearer '+TOKEN}};if(body)opts.body=JSON.stringify(body);try{const r=await fetch('/sshws-api'+path,opts);return await r.json();}catch(e){return{error:e.message};}}
