...
menu_1() {
  ...
  # ── 30% หยุด x-ui ก่อน set credential + basepath ผ่าน sqlite3 ──
  rgb_bar 30 "ตั้งค่า credential + basepath..."
  systemctl stop x-ui 2>/dev/null || true
  sleep 2

  local _xui_db="/etc/x-ui/x-ui.db"
  # generate basepath ใหม่เสมอ (installer gen เองไม่ได้ให้เรา control)
  local _gen_bp; _gen_bp="/$(openssl rand -hex 6)/"
  echo "$_gen_bp" > /etc/chaiya/xui-basepath.conf

  # set credential ด้วย bcrypt + set port + basepath ลง db โดยตรง
  local _pw_hash=""
  _pw_hash=$(python3 -c "
import bcrypt, sys
...
" "$_pw" 2>/dev/null)
  ...
  # fallback CLI ถ้า bcrypt ล้มเหลว
  if [[ -z "$_pw_hash" ]]; then
    /usr/local/x-ui/x-ui setting -username "$_u" -password "$_pw" 2>/dev/null || true
  fi
