#!/bin/bash
# ============================================================
#  CHAIYA V2RAY PRO MAX v10 (script v35)  —  Full Auto Setup
...
echo -e "${R2}🔥 กำลังติดตั้ง CHAIYA V2RAY PRO MAX v10 (script v35)...${RS}"
...
menu_16() {
  ...
  # ── เวอร์ชันปัจจุบัน ──────────────────────────────────────
  local cur_ver
  cur_ver=$(grep -m1 'CHAIYA V2RAY PRO MAX v' /usr/local/bin/menu 2>/dev/null \
  ...
  case $_sub16 in
    ...
    1)
      ...
      if curl -Ls "$REPO_URL" -o "$tmp_script" 2>/dev/null && [[ -s "$tmp_script" ]]; then
        local new_ver
        new_ver=$(grep -m1 'CHAIYA V2RAY PRO MAX v' "$tmp_script" 2>/dev/null \
