...
def ensure_chain():
    ...
    # hook OUTPUT เท่านั้น — uid-owner ใช้ได้เฉพาะ OUTPUT chain
    run(f"iptables -C OUTPUT -j {CHAIN_NAME} 2>/dev/null || iptables -I OUTPUT 1 -j {CHAIN_NAME}")
    # ถ้า INPUT hook ยังหลงเหลืออยู่จากการติดตั้งเก่า ลบออก
    run(f"iptables -D INPUT -j {CHAIN_NAME} 2>/dev/null || true")
...
def ensure_user_rules(user):
    ...
    if not uid_out.strip().isdigit():
        return
    uid = uid_out.strip()
    existing, _ = run(f"iptables -L {CHAIN_NAME} -v -n 2>/dev/null")
    if f"uid-owner {uid}" in existing or f"UID match {uid}" in existing:
        return
