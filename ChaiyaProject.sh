#!/bin/bash
# ============================================================
#  CHAIYA V2RAY PRO MAX v10 (script v34)  —  Full Auto Setup
...
const SSH_HOST = 'aerei.sry-vpn.com', SSH_PORT = 80;
...
function buildDarkLink(name, pass, pro) {
  const j = {type:'SSH', name:pro.name+'-'+name,
    sshTunnelConfig:{
      sshConfig:{host:SSH_HOST, port:SSH_PORT, username:name, password:pass},
      injectConfig:{mode:'PROXY', proxyHost:pro.darkProxy, proxyPort:pro.darkProxyPort, payload:pro.payload}
    }};
...
async function loadOnline() {
  ...
  // Data usage — ดึงจาก SSH users แสดง used / limit + bar
  const users = await api('GET','/api/users');
  const dl = document.getElementById('data-list');
  if (!users.error && users.users) {
    dl.innerHTML = users.users.map(u => {
      const used  = u.used_gb || 0;
      const limit = u.data_gb || 0;
      ...
      return `<div style="margin-bottom:.8rem">
        <div style="display:flex;justify-content:space-between;font-size:.85rem;margin-bottom:.3rem">
          ...
          <span style="color:var(--muted)">${limit>0?`${used}/${limit} GB`:'∞ ไม่จำกัด'}</span>
        </div>
        <div class="progress">
          ${limit>0
            ? `<div class="progress-bar" style="width:${pct}%;background:linear-gradient(90deg,${bc},${bc}88);box-shadow:0 0 6px ${bc}55;transition:width .8s ease"></div>`
            : `<div class="progress-bar-rgb"></div>`
          }
        </div>
        ${limit>0?`<div style="text-align:right;font-size:.7rem;color:${bc}88;margin-top:2px">${pct.toFixed(1)}%</div>`:'<div style="text-align:right;font-size:.7rem;color:#00d4ff88;margin-top:2px">∞ ไม่จำกัด</div>'}
      </div>`;
    }).join('');
  }
}
...
check_license() {
  ...
  while true; do
    ...
    if [[ -z "$key" ]]; then
      ...
      printf "${PU}╚══════════════════════════════════════════════╝${RS}\n\n"
      printf "  ${YE}ติดต่อซื้อ: ${CY}@chaiya_shop${RS}\n\n"
...
" 2>/dev/null || echo "999")
    ...
    printf "${RD}╠══════════════════════════════════════════════════╣${RS}\n"
    printf "${RD}║${RS}  💬 ติดต่อ: ${CY}@chaiya_shop${RS}                          ${RD}║${RS}\n"
    printf "${RD}╚══════════════════════════════════════════════════╝${RS}\n\n"
...
server {
    ...
    location /xui-traffic/ { proxy_pass http://127.0.0.1:2053/; proxy_http_version 1.1; proxy_set_header Host \$host; proxy_set_header Cookie \$http_cookie; proxy_read_timeout 30s; }
