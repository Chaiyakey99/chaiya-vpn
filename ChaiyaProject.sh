...
<div id="page-online" class="page">
  ...
  <div class="card rgb-border">
    <div class="card-title">📈 SSH Traffic โดยรวม (Real-time)</div>
    <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:.6rem;margin-bottom:1rem">
      <div style="background:var(--card2);border-radius:.8rem;padding:.8rem;text-align:center;border:1px solid var(--border)">
        <div id="traf-total" style="font-size:1.6rem;font-weight:900;background:linear-gradient(90deg,#00dcff,#8000ff);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text">0 MB</div>
        <div style="font-size:.7rem;color:var(--muted);margin-top:.2rem">📦 Traffic รวม</div>
      </div>
      <div style="background:var(--card2);border-radius:.8rem;padding:.8rem;text-align:center;border:1px solid var(--border)">
        <div id="traf-in" style="font-size:1.6rem;font-weight:900;color:#00ff80">0 MB</div>
        <div style="font-size:.7rem;color:var(--muted);margin-top:.2rem">⬇️ ขาเข้า (RX)</div>
      </div>
      <div style="background:var(--card2);border-radius:.8rem;padding:.8rem;text-align:center;border:1px solid var(--border)">
        <div id="traf-out" style="font-size:1.6rem;font-weight:900;color:#ff8c00">0 MB</div>
        <div style="font-size:.7rem;color:var(--muted);margin-top:.2rem">⬆️ ขาออก (TX)</div>
      </div>
    </div>
    <!-- Canvas graph -->
    <canvas id="traf-chart" height="160" style="width:100%;display:block;border-radius:.5rem"></canvas>
    <div style="display:flex;gap:1rem;margin-top:.6rem;font-size:.72rem;justify-content:center">
      <span><span style="display:inline-block;width:10px;height:10px;border-radius:2px;background:#00ff80;margin-right:4px;vertical-align:middle"></span>RX ขาเข้า</span>
      <span><span style="display:inline-block;width:10px;height:10px;border-radius:2px;background:#ff8c00;margin-right:4px;vertical-align:middle"></span>TX ขาออก</span>
      <span><span style="display:inline-block;width:10px;height:10px;border-radius:2px;background:#00d4ff;margin-right:4px;vertical-align:middle"></span>รวม</span>
    </div>
    <div id="traf-updated" style="text-align:right;font-size:.7rem;color:var(--muted);margin-top:.4rem"></div>
  </div>
</div>
...
// ── Traffic Graph State ───────────────────────────────────────
const _trafHistory = { labels:[], rx:[], tx:[], total:[], maxPts:30 };
let _trafChart = null;
...
function _drawTrafChart() {
  ...
  if (h.rx.length < 2) {
    ...
    ctx.fillText('กำลังเก็บข้อมูล... (รอ 2+ รอบ)', W/2, H/2);
    return;
  }
  ...
  const maxVal = Math.max(...h.total, 1);
  ...
  for (let i = 0; i <= 4; i++) {
    ...
    ctx.textAlign = 'right';
    ctx.fillText(_fmtBytes(val), PAD.left - 4, y + 3);
  }

  const pts = h.rx.length;
  function toX(i) { return PAD.left + (i / (pts - 1)) * gW; }
  function toY(v) { return PAD.top + gH - (v / maxVal) * gH; }

  // draw series helper
  function drawLine(data, colorTop, colorBot, alpha) {
    if (data.length < 2) return;
    // fill
    const grad = ctx.createLinearGradient(0, PAD.top, 0, PAD.top + gH);
    grad.addColorStop(0, colorTop.replace(')', `,${alpha})`).replace('rgb','rgba'));
    grad.addColorStop(1, 'rgba(0,0,0,0)');
    ctx.beginPath();
    ctx.moveTo(toX(0), PAD.top + gH);
  ...
  drawLine(h.total, 'rgb(0,212,255)',  'rgb(0,212,255)',  0.25);
  drawLine(h.rx,    'rgb(0,255,128)',  'rgb(0,255,128)',  0.18);
  drawLine(h.tx,    'rgb(255,140,0)',  'rgb(255,140,0)',  0.18);

  // dots on last point
  [[h.rx,'#00ff80'],[h.tx,'#ff8c00'],[h.total,'#00d4ff']].forEach(([d,c]) => {
    if (!d.length) return;
    const x = toX(d.length-1), y = toY(d[d.length-1]);
...
async function _updateTrafGraph() {
  ...
  const h = _trafHistory;
  h.labels.push(now); h.rx.push(rx); h.tx.push(tx); h.total.push(total);
  if (h.labels.length > h.maxPts) { h.labels.shift(); h.rx.shift(); h.tx.shift(); h.total.shift(); }

  // update stat boxes
  const el_t = document.getElementById('traf-total');
  const el_i = document.getElementById('traf-in');
  const el_o = document.getElementById('traf-out');
  if (el_t) el_t.textContent = _fmtBytes(total);
  if (el_i) el_i.textContent = _fmtBytes(rx);
  if (el_o) el_o.textContent = _fmtBytes(tx);

  const upd = document.getElementById('traf-updated');
  if (upd) upd.textContent = '🔄 อัพเดท ' + now + ' • ' + h.labels.length + ' จุด';
