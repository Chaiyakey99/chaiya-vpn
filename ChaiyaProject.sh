...
SSLEOF
    ...
    # 3x-ui ใหม่ใช้ field: webPublicKeyFile / webPrivateKeyFile
    # (เวอร์ชันเก่าใช้ webCertFile / webKeyFile — ลองทั้งคู่)
    local _xui_cert_payload
    _xui_cert_payload=$(python3 -c "
import json
print(json.dumps({
  'webPublicKeyFile': '${_cert_path}',
  'webPrivateKeyFile': '${_key_path}'
}))
")
    local _xui_cert_res
    _xui_cert_res=$(xui_api POST "/panel/api/setting/update" "$_xui_cert_payload" 2>/dev/null)
    # fallback: ลองชื่อ field เก่าถ้าใหม่ไม่สำเร็จ
...
menu_16() {
  ...
  # ── เวอร์ชันปัจจุบัน (ใช้ md5 ของไฟล์ menu แทน version string) ──
  local cur_ver
  cur_ver=$(md5sum /usr/local/bin/menu 2>/dev/null | cut -c1-8 || echo "ไม่ทราบ")
  local cur_date
    ...
    1)
      ...
      if curl -Ls "$REPO_URL" -o "$tmp_script" 2>/dev/null && [[ -s "$tmp_script" ]]; then
        ...
        if [[ "$_cf" == "y" || "$_cf" == "Y" ]]; then
          ...
          # รันจากไฟล์ที่ดาวน์โหลดมาแล้ว — ไม่ดาวน์โหลดซ้ำ
          chmod +x "$tmp_script"
          bash "$tmp_script"
          rm -f "$tmp_script"
          exit 0
