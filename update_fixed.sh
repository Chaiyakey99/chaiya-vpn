#!/bin/bash
# ============================================================
#   CHAIYA VPN PANEL - Update Script
#   รันคำสั่งเดียว: bash update.sh
# ============================================================
...
XUI_PASS=$(cat /etc/chaiya/xui-pass.conf 2>/dev/null || echo "admin")
XUI_PORT=$(cat /etc/chaiya/xui-port.conf 2>/dev/null || echo "2053")
PANEL_PASS_HASH=$(cat /etc/chaiya/panel-pass.hash 2>/dev/null || echo "")
...
async function xuiLogin(){
  const SESSION_KEY = 'chaiya_auth';
  let user = CFG.xui_user || 'admin', pass = CFG.xui_pass || 'admin';
  try {
    const s = JSON.parse(sessionStorage.getItem(SESSION_KEY)||'{}');
    if(s.xui_user) user = s.xui_user;
    if(s.xui_pass) pass = s.xui_pass;
    if(s.user && !s.xui_user) user = s.user;
    if(s.pass && !s.xui_pass) pass = s.pass;
  } catch(e){}
...
function buildVlessLink(uuid,host,port,sni,email){
  return `vless://${uuid}@${host}:${port}?path=${encodeURIComponent('/chaiya')}&security=none&encryption=none&host=${encodeURIComponent(sni)}&type=ws#CHAIYA-${email}-${port}`;
}
