...
rgb_bar() {
  ...
  printf "\r  ${bar_color}[%s%s]${RS} ${WH}%3d%%${RS} ${YE}%s${RS}  " \
...
"
  ...
  # ════════════════════════════════════════════════════════════
  # STEP 1 — ถาม Username/Password พร้อม skull ☠️
  # ════════════════════════════════════════════════════════════
  printf "\n${R1}╔══════════════════════════════════════════════════╗${RS}\n"
  printf "${R1}║${RS}  ☠️  ${WH}STEP 1/5  ตั้งค่า Credential สำหรับ 3x-ui  ${R1}║${RS}\n"
  printf "${R1}╚══════════════════════════════════════════════════╝${RS}\n\n"
  rgb_bar 5 "รอ input..."; printf "\n\n"

  read -rp "$(printf "  ${YE}☠️  Username admin: ${RS}")" _u
  [[ -z "$_u" ]] && _u="admin"

  local _pw _pw2
  while true; do
    read -rsp "$(printf "  ${YE}☠️  Password admin (ห้ามว่าง): ${RS}")" _pw; echo ""
    [[ -n "$_pw" ]] && break
    printf "  ${RD}✗ Password ห้ามว่าง กรอกใหม่${RS}\n"
  done
  read -rsp "$(printf "  ${YE}☠️  ยืนยัน Password: ${RS}")" _pw2; echo ""
  if [[ "$_pw" != "$_pw2" ]]; then
  ...
  chmod 600 /etc/chaiya/xui-user.conf /etc/chaiya/xui-pass.conf
  printf "\n"; rgb_bar 15 "บันทึก credential สำเร็จ ✔"; printf "\n\n"
  printf "  ${GR}✔ User: ${WH}%s${GR}  |  Pass: ****${RS}\n\n" "$_u"

  # ════════════════════════════════════════════════════════════
  # STEP 2 — ติดตั้ง 3x-ui อัตโนมัติ (ไม่ถามผู้ใช้เพิ่ม)
  # ════════════════════════════════════════════════════════════
  printf "${R1}╔══════════════════════════════════════════════════╗${RS}\n"
  printf "${R1}║${RS}  ☠️  ${WH}STEP 2/5  ติดตั้ง 3x-ui                    ${R1}║${RS}\n"
  printf "${R1}╚══════════════════════════════════════════════════╝${RS}\n"
  printf "  ${CY}Port panel : 2053  |  SSL : IP cert  |  ACME : 80${RS}\n\n"
  rgb_bar 20 "ดาวน์โหลด install script..."; printf "\n\n"
  ...
  rgb_bar 30 "กำลังติดตั้ง 3x-ui..."; printf "\n\n"

  # ตอบ prompt ทั้งหมดอัตโนมัติ: y=ยืนยัน, 2053=port, 2=IP cert, ""=ข้าม IPv6, 80=ACME port
  printf "y\n2053\n2\n\n80\n" | bash "$_xui_sh" 2>&1 | tee /var/log/chaiya-xui-install.log | \
    while IFS= read -r line; do
      printf "  ${R6}›${RS} %s\n" "$line"
    done
  rm -f "$_xui_sh"

  rgb_bar 50 "ติดตั้งเสร็จ รอ service..."; printf "\n\n"
  sleep 4

  # ════════════════════════════════════════════════════════════
  # STEP 3 — Reset credential ผ่าน x-ui CLI
  # ════════════════════════════════════════════════════════════
  printf "${R1}╔══════════════════════════════════════════════════╗${RS}\n"
  printf "${R1}║${RS}  ☠️  ${WH}STEP 3/5  Reset Credential + Detect BasePath ${R1}║${RS}\n"
  printf "${R1}╚══════════════════════════════════════════════════╝${RS}\n\n"
  rgb_bar 55 "Reset credential..."; printf "\n\n"

  /usr/local/x-ui/x-ui setting -username "$_u" -password "$_pw" 2>/dev/null || \
    x-ui setting -username "$_u" -password "$_pw" 2>/dev/null || \
    printf "  ${YE}⚠ CLI reset ไม่สำเร็จ — จะใช้ API แทน${RS}\n"

  systemctl restart x-ui 2>/dev/null || true
  sleep 3

  # detect webBasePath จาก sqlite3
  local _basepath
  _basepath=$(detect_xui_basepath)
  echo "$_basepath" > /etc/chaiya/xui-basepath.conf
  printf "  ${GR}✔ webBasePath : ${WH}%s${RS}\n" "$_basepath"

  # อัพเดต port จาก config
  local _xp
  ...
  rgb_bar 65 "Credential reset สำเร็จ ✔"; printf "\n\n"
  printf "  ${GR}✔ Panel Port  : ${WH}%s${RS}\n" "$_panel_port"

  # ════════════════════════════════════════════════════════════
  # STEP 4 — เช็ค port 2053 (retry 10 รอบ × 3 วินาที) + Login API
  # ════════════════════════════════════════════════════════════
  printf "\n${R1}╔══════════════════════════════════════════════════╗${RS}\n"
  printf "${R1}║${RS}  ☠️  ${WH}STEP 4/5  เช็ค Port 2053 + Login API         ${R1}║${RS}\n"
  printf "${R1}╚══════════════════════════════════════════════════╝${RS}\n\n"

  local _ok=0
  for _i in $(seq 1 10); do
    local _pct=$(( 65 + _i * 2 ))
    rgb_bar "$_pct" "ลอง port 2053 รอบที่ ${_i}/10..."
    if curl -s --max-time 3 "http://127.0.0.1:2053/" &>/dev/null; then
      _ok=1; break
    fi
    sleep 3
  done
  ...
  if [[ "$_ok" == "1" ]]; then
    printf "  ${GR}✔ Port 2053 ตอบสนองแล้ว!${RS}\n"
  else
    printf "  ${YE}⚠ Port 2053 ยังไม่ตอบ — ลอง restart x-ui${RS}\n"
    systemctl restart x-ui 2>/dev/null || true
    sleep 5
  fi

  # Login API + แสดง URL panel
  rgb_bar 85 "Login API..."; printf "\n\n"
  if xui_login 2>/dev/null; then
    printf "  ${GR}✔ Login API สำเร็จ!${RS}\n"
  else
    printf "  ${YE}⚠ Login API ไม่สำเร็จ (x-ui อาจยังโหลด) — ดำเนินต่อ${RS}\n"
  fi

  printf "\n  ${R2}┌─────────────────────────────────────────────┐${RS}\n"
  printf "  ${R2}│${RS}  🌐 Panel URL  : ${WH}http://%s:%s${RS}\n" "$MY_IP" "$_panel_port"
  printf "  ${R2}│${RS}  👤 Username   : ${WH}%s${RS}\n" "$_u"
  printf "  ${R2}│${RS}  🔑 Password   : ${WH}%s${RS}\n" "$_pw"
  printf "  ${R2}│${RS}  📂 BasePath   : ${WH}%s${RS}\n" "$_basepath"
  printf "  ${R2}└─────────────────────────────────────────────┘${RS}\n\n"

  # ════════════════════════════════════════════════════════════
  # STEP 5 — สร้าง VMess inbound 8080 + 8880 อัตโนมัติ
  # ════════════════════════════════════════════════════════════
  printf "${R1}╔══════════════════════════════════════════════════╗${RS}\n"
  printf "${R1}║${RS}  ☠️  ${WH}STEP 5/5  สร้าง VMess Inbounds อัตโนมัติ    ${R1}║${RS}\n"
  printf "${R1}╚══════════════════════════════════════════════════╝${RS}\n\n"

  # รายการ port:remark:sni
  local _inbounds=(
  ...
  for _item in "${_inbounds[@]}"; do
    (( _step_n++ ))
    local _pp; _pp=$(echo "$_item" | cut -d: -f1)
    local _rr; _rr=$(echo "$_item" | cut -d: -f2)
    local _ss; _ss=$(echo "$_item" | cut -d: -f3-)
    local _uid; _uid=$(cat /proc/sys/kernel/random/uuid)

    local _bar_pct=$(( 88 + _step_n * 5 ))
    rgb_bar "$_bar_pct" "สร้าง inbound port ${_pp}..."; printf "\n"
  ...
  "sniffing":{"enabled":true,"destOverride":["http","tls"]}
    ...
    if echo "$_res" | grep -q '"success":true'; then
      printf "  ${GR}✔ Port ${WH}%s${GR} (%s)${RS}\n" "$_pp" "$_rr"
      printf "     SNI  : ${YE}%s${RS}\n" "$_ss"
      printf "     UUID : ${CY}%s${RS}\n\n" "$_uid"
    else
      printf "  ${YE}⚠ Port %s — API ไม่ตอบ (อาจสร้างซ้ำหรือ x-ui ยังไม่พร้อม)${RS}\n\n" "$_pp"
    fi
  done
  ...
  # สรุปผล
  # ════════════════════════════════════════════════════════════
  rgb_bar 100 "เสร็จสมบูรณ์! ✔"; printf "\n\n"
  printf "${R1}╔══════════════════════════════════════════════════╗${RS}\n"
  printf "${R1}║${RS}  ${GR}✅  ติดตั้งและตั้งค่าทั้งหมดเสร็จสมบูรณ์!       ${R1}║${RS}\n"
  printf "${R1}║${RS}  🌐 ${WH}http://%-37s${R1}║${RS}\n" "${MY_IP}:${_panel_port}"
  printf "${R1}║${RS}  👤 User: ${WH}%-8s${RS}  🔑 Pass: ${WH}%-18s${R1}║${RS}\n" "$_u" "$_pw"
  printf "${R1}║${RS}  📡 VMess 8080 (AIS)  |  VMess 8880 (TRUE)     ${R1}║${RS}\n"
  printf "${R1}║${RS}  ${YE}➡ เมนู 3 สร้าง VLESS user ได้เลย!            ${R1}║${RS}\n"
  printf "${R1}╚══════════════════════════════════════════════════╝${RS}\n\n"
  read -rp "$(printf "${YE}Enter ย้อนกลับเมนูหลัก...${RS}")"
}
