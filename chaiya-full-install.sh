...
# ============================================================
#   CHAIYA V2RAY PRO MAX - FULL INSTALLER v2
# ============================================================
...
# สร้าง Xray config ด้วย python3 แทน heredoc
python3 << 'PYEOF'
import json
...
print("✅ สร้าง Xray config แล้ว")
PYEOF

# สร้าง xray service ด้วย python3
python3 << 'PYEOF'
svc = """[Unit]
...
with open("/etc/systemd/system/xray.service", "w") as f:
    f.write(svc)
print("✅ สร้าง xray service แล้ว")
PYEOF

# สร้าง reload-users script ด้วย python3
python3 << 'PYEOF'
script = """#!/bin/bash
sleep 3
python3 << 'EOF'
...
print("✅ สร้าง reload-users script แล้ว")
PYEOF
...
# สร้าง chaiya script หลักด้วย python3 หลีกเลี่ยง heredoc ซ้อน
python3 << 'PYEOF'
script = r"""#!/bin/bash
...
"
  echo "{\"inboundTag\":\"vless-in\",\"user\":{\"email\":\"$NAME\",\"level\":0,\"id\":\"$UUID\"}}" > "$TMPJSON"
...
open('/etc/nginx/sites-available/chaiya','w').write(cfg)
"
...
print("✅ สร้าง chaiya script แล้ว")
PYEOF
