...
# ============================================================
#   CHAIYA V2RAY PRO MAX - FULL INSTALLER
#   รวมทุกอย่างในไฟล์เดียว พร้อมใช้งานบน VPS ใหม่
# ============================================================
...
# สร้าง Xray config
cat > /usr/local/etc/xray/confs/00_base.json << 'CONF'
{
  "log": {
...
cp /usr/local/etc/xray/confs/00_base.json /usr/local/etc/xray/config.json
...
# สร้าง reload-users script
cat > /usr/local/bin/chaiya-reload-users << 'RELOAD'
#!/bin/bash
sleep 3
python3 << 'EOF'
    ...
    for inbound in cfg.get('inbounds', []):
        if inbound.get('protocol') == 'vless':
            for client in inbound['settings']['clients']:
                data = {"inboundTag":"vless-in","user":{"email":client['email'],"level":0,"id":client['id']}}
                with open('/tmp/u.json','w') as f: json.dump(data,f)
                subprocess.run(['/usr/local/bin/xray','api','adu','--server=127.0.0.1:10085','/tmp/u.json'],capture_output=True)
except: pass
EOF
RELOAD
chmod +x /usr/local/bin/chaiya-reload-users
...
add_uuid_to_xray() {
  local UUID="$1" NAME="$2"
  local TMPJSON="/tmp/chaiya_${NAME}.json"
  python3 -c "
import json, shutil
path = '$CONFIG'
...
"
  cat > "$TMPJSON" << JSONEOF
{"inboundTag":"vless-in","user":{"email":"$NAME","level":0,"id":"$UUID"}}
JSONEOF
  if /usr/local/bin/xray api adu --server=127.0.0.1:10085 "$TMPJSON" 2>/dev/null; then
    printf "${GR}✅ เพิ่มผ่าน API สำเร็จ (ไม่ restart)${RS}\n"
  else
    printf "${YE}⚠️  กำลัง restart xray...${RS}\n"
...
NGX
  ...
  3)
    ...
    SHORT=$(curl -s "https://tinyurl.com/api-create.php?url=$(python3 -c "import urllib.parse; print(urllib.parse.quote('$LINK'))")" 2>/dev/null || echo "")
    clear
    printf "${YE}================================${RS}\n"
...
" 2>/dev/null
  ...
  13)
    while true; do
      ...
      3) printf "DEFAULT: $(grep '^DEFAULT ' $LIMIT_FILE                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       