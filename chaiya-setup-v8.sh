...
# certbot ใช้ port 8888 (ไม่แตะ port 80 ที่ WS-Stunnel/SSH ใช้อยู่)
if command -v certbot &>/dev/null; then
  for _try in 1 2 3; do
...
# ── CERTBOT AUTO-RENEW ────────────────────────────────────────
[[ $USE_SSL -eq 1 ]] && \
  (crontab -l 2>/dev/null; echo "0 3 * * * certbot renew --quiet --standalone --http-01-port 8888 && systemctl reload nginx") | sort -u | crontab -
...
echo ""
if [[ $USE_SSL -eq 1 ]]; then
  echo -e "  🌐 Panel URL   : ${CYAN}${BOLD}https://${DOMAIN}:81${NC}"
