...
# ── DETECT UPDATE MODE ───────────────────────────────────────
UPDATE_MODE=0
if [[ -f /etc/chaiya/domain.conf && -f /etc/chaiya/xui-user.conf ]]; then
  UPDATE_MODE=1
  DOMAIN=$(cat /etc/chaiya/domain.conf)
  XUI_USER=$(cat /etc/chaiya/xui-user.conf)
  XUI_PASS=$(cat /etc/chaiya/xui-pass.conf 2>/dev/null || echo "")
  ok "พบการติดตั้งเดิม — UPDATE MODE"
  ok "โดเมน: ${CYAN}$DOMAIN${NC}"
  ok "User: ${CYAN}$XUI_USER${NC}"
else
# ── DOMAIN SETUP ─────────────────────────────────────────────
echo ""
echo -e "${YELLOW}════════════════════════════════════════${NC}"
