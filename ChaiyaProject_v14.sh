...
[Service]
Type=simple
ExecStart=/usr/local/bin/websocat --binary -s 0.0.0.0:80 tcp:127.0.0.1:22
Restart=always
RestartSec=3
...
'body{background:#0a0a0f;font-family:Segoe UI,sans-serif;min-height:100vh;display:flex;align-items:center;justify-content:center;padding:16px}'
'.wrap{width:100%;max-width:420px}'
'@keyframes rgbwave{0%{border-color:#ff0080}16%{border-color:#ff8000}33%{border-color:#ffee00}50%{border-color:#00ff80}66%{border-color:#00d4ff}83%{border-color:#b400ff}100%{border-color:#ff0080}}'
...
'@keyframes rgbdiv{0%{background:#ff008066}33%{background:#00ff8066}66%{background:#00d4ff66}100%{background:#ff008066}}'
'.header{text-align:center;padding:22px 0 14px;border-bottom:2px solid #1a1a2e;margin-bottom:6px}'
'.header .fire{font-size:32px;animation:pulse 2s infinite}'
'.header .title{font-size:22px;font-weight:700;letter-spacing:6px;animation:rgbtxt 4s linear infinite}'
'.header .user{margin-top:6px;color:#7c9cbf;font-size:14px}'
'.header .user span{color:#00cfff}'
'.divider{height:2px;margin:4px 0 10px;animation:rgbdiv 4s linear infinite;border-radius:2px}'
'.row{display:flex;align-items:flex-start;padding:11px 14px;border-radius:8px;margin-bottom:8px;background:#0f0f1a;border:1.5px solid #1a1a2e;gap:10px;animation:rgbwave 4s linear infinite}'
'.row .ico{font-size:18px;min-width:24px;text-align:center;margin-top:2px}'
'.row .content{flex:1;min-width:0}'
'.row .lbl{font-size:10px;color:#4a6a8a;letter-spacing:2px;text-transform:uppercase;margin-bottom:3px}'
'.row .val{font-size:13px;color:#c8e0f0;word-break:break-all;line-height:1.4}'
'.row.exp .val{color:#ff6060;font-weight:600}'
'.row.data .val{color:#00cfff}'
'.row.proto .val{color:#a0c0ff}'
'.link-box{background:#0a0a14;border-radius:8px;border:1.5px solid #1a1a2e;padding:12px 14px;margin-bottom:10px;animation:rgbwave 4s linear infinite}'
'.link-box .lbl{font-size:10px;color:#4a6a8a;letter-spacing:2px;text-transform:uppercase;margin-bottom:6px}'
'.link-box .val{font-size:11px;color:#7a9abf;word-break:break-all;line-height:1.6;font-family:monospace}'
'.btn-copy{width:100%;padding:14px;border:none;border-radius:10px;font-size:15px;font-weight:700;letter-spacing:1px;cursor:pointer;margin-bottom:10px;background:linear-gradient(135deg,#b400ff,#5500cc);color:#fff}'
'.btn-copy:active{opacity:.7}'
'.btn-qr{width:100%;padding:13px;border-radius:10px;border:1.5px solid #b8860b;background:transparent;color:#ffd700;font-size:14px;font-weight:600;cursor:pointer;letter-spacing:1px}'
'#qrbox{display:none;margin-top:10px;text-align:center;background:#fff;padding:12px;border-radius:10px}'
'.toast{position:fixed;bottom:30px;left:50%;transform:translateX(-50%);background:#00ff80;color:#000;padding:10px 24px;border-radius:20px;font-weight:700;font-size:13px;opacity:0;transition:opacity .3s;pointer-events:none}'
'.toast.show{opacity:1}'
'</style></head><body><div class=\"wrap\">'
'<div class=\"header\"><div class=\"fire\">\U0001f525</div>'
'<div class=\"title\">CHAIYA VPN</div>'
'<div class=\"user\">\U0001f464 <span>' + u + '</span></div></div>'
'<div class=\"divider\"></div>'
'<div class=\"row exp\"><div class=\"ico\">\U0001f4c5</div><div class=\"content\"><div class=\"lbl\">\u0e2b\u0e21\u0e14\u0e2d\u0e32\u0e22\u0e38</div><div class=\"val\">' + ex + '</div></div></div>'
'<div class=\"row data\"><div class=\"ico\">\U0001f4ca</div><div class=\"content\"><div class=\"lbl\">Data</div><div class=\"val\">Unlimited</div></div></div>'
'<div class=\"row proto\"><div class=\"ico\">\U0001f310</div><div class=\"content\"><div class=\"lbl\">Protocol</div><div class=\"val\">VLESS WS</div></div></div>'
'<div class=\"row\"><div class=\"ico\">\U0001f5a5</div><div class=\"content\"><div class=\"lbl\">Host / IP</div><div class=\"val\">' + hv + '</div></div></div>'
'<div class=\"row\"><div class=\"ico\">\U0001f50c</div><div class=\"content\"><div class=\"lbl\">Port</div><div class=\"val\">' + pv + '</div></div></div>'
'<div class=\"row\"><div class=\"ico\">\U0001f512</div><div class=\"content\"><div class=\"lbl\">SNI / Bug Host</div><div class=\"val\">' + sv + '</div></div></div>'
'<div class=\"link-box\"><div class=\"lbl\">VLESS Link</div><div class=\"val\" id=\"vlink\">' + lnk + '</div></div>'
...
'<script>'
'function copyLink(){var t=document.getElementById(\"vlink\").textContent;navigator.clipboard.writeText(t).then(function(){var el=document.getElementById(\"toast\");el.classList.add(\"show\");setTimeout(function(){el.classList.remove(\"show\")},1800);});}'
'var qrDone=false;'
'function toggleQR(){var box=document.getElementById(\"qrbox\");if(box.style.display===\"block\"){box.style.display=\"none\";return;}box.style.display=\"block\";if(!qrDone){new QRCode(box,{text:\"' + lnk.replace('\"','\\\\\"') + '\",width:240,height:240,colorDark:\"#000\",colorLight:\"#fff\"});qrDone=true;}}'
'</script></body></html>'
)
with open(sys.argv[8], 'w') as f:
    f.write(html)
" "$uname" "$link" "$uuid" "$host_val" "$port_val" "$sni_val" "$exp" "$outfile"
...
import json, sys
print(json.dumps({
  'clients': [{'id': sys.argv[1], 'flow': '', 'email': sys.argv[2], 'limitIp': 0, 'totalGB': 0, 'expiryTime': 0, 'enable': True, 'comment': '', 'reset': 0}],
  'decryption': 'none'
}))
" "$_ibuid" "chaiya-${_ibremark}")
    _stream=$(python3 -c "
import json, sys
print(json.dumps({
  'network': 'ws',
  'security': 'none',
  'wsSettings': {'path': '/', 'headers': {'Host': sys.argv[1]}}
}))
" "$_ibsni")
    _sniff=$(python3 -c "import json; print(json.dumps({'enabled':True,'destOverride':['http','tls']}))")
