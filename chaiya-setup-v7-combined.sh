...
def get_banned_users():
    ...
    try:
        ...
        if os.path.exists(find_xui_db()):
            con = sqlite3.connect(find_xui_db())
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path == '/api/status':
            ...
            # ดึง VLESS online จาก active TCP connections บน port 8080/8880
            # แล้ว map email จาก DB — ไม่ต้อง login x-ui (หลีกเลี่ยง bcrypt issue)
            import sqlite3 as _sq3
            emails = []
            try:
                _, c8080, _ = run_cmd(
                    "ss -tn state established 2>/dev/null | awk '{print $4}' | grep -c ':8080$' || echo 0"
                )
                _, c8880, _ = run_cmd(
                    "ss -tn state established 2>/dev/null | awk '{print $4}' | grep -c ':8880$' || echo 0"
                )
                n8080 = int(c8080.strip().split()[0]) if c8080.strip() else 0
                n8880 = int(c8880.strip().split()[0]) if c8880.strip() else 0

                if n8080 + n8880 > 0:
                    _db = find_xui_db()
                    if os.path.exists(_db):
                        con = _sq3.connect(_db)
                        # ลอง client_traffics — email ที่ใช้งานจริงจะมี up+down > 0
                        got = False
                        ...
                        # fallback: เอา email ทั้งหมดที่ enable=true ใน inbound ที่มี connection
                        if not got or not emails:
                            try:
                                import json as _jj
                                ports_with_conn = []
                                if n8080 > 0: ports_with_conn.append(8080)
                                if n8880 > 0: ports_with_conn.append(8880)
                                placeholders = ','.join('?' for _ in ports_with_conn)
                                rows = con.execute(
                                    f"SELECT settings FROM inbounds WHERE enable=1 AND port IN ({placeholders})",
                                    ports_with_conn
                                ).fetchall()
                                for (settings_str,) in rows:
                                    try:
                                        s = _jj.loads(settings_str)
                                        for c in s.get('clients', []):
                                            if c.get('enable', True):
                                                em = c.get('email') or c.get('id', '')
