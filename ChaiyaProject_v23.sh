...
# ── Port 80: WebSocket SSH tunnel → SSH port 22
# timeout 7d รองรับ long-lived WS connection
server {
    listen 80;
    server_name _;
    ...
    # timeout 7 วัน (604800 วินาที)
    proxy_read_timeout  604800s;
    proxy_send_timeout  604800s;
    proxy_connect_timeout 10s;

    # ทุก path → websocat listener ที่ UNIX socket
    location / {
        proxy_pass http://unix:/var/run/chaiya-sshws.sock;
    }
}
NGINXWS
  ...
  printf "${GR}✅ nginx WebSocket block port 80 → UNIX socket พร้อมแล้ว${RS}\n"
}
...
[Unit]
Description=Chaiya SSH WebSocket Tunnel (websocat → SSH :22)
After=network.target
Wants=network.target

[Service]
Type=simple
# websocat รับ WebSocket จาก nginx via UNIX socket แล้ว forward ไป SSH port 22
ExecStart=/usr/local/bin/websocat \
  --binary \
  --unix-listen=/var/run/chaiya-sshws.sock \
  tcp:127.0.0.1:22
Restart=always
RestartSec=3
# สิทธิ์ socket ให้ nginx อ่านได้
ExecStartPost=/bin/chmod 777 /var/run/chaiya-sshws.sock
StandardOutput=append:/var/log/chaiya-sshws.log
StandardError=append:/var/log/chaiya-sshws.log
...
SVCEOF
  ...
  systemctl restart chaiya-sshws 2>/dev/null
  sleep 1
  if systemctl is-active --quiet chaiya-sshws; then
    printf "${GR}✅ chaiya-sshws (websocat) service ทำงานแล้ว${RS}\n"
  else
    printf "${RD}❌ chaiya-sshws ยังไม่ active — ดู log: journalctl -u chaiya-sshws -n 20${RS}\n"
  fi
}
...
_m18_show_appconfig() {
  ...
  printf "${CY}║${RS}  ${YE}Port    ${WH}: 80                                       ${CY}║${RS}\n"
  printf "${CY}║${RS}  ${YE}Mode    ${WH}: WebSocket (WS)                           ${CY}║${RS}\n"
  printf "${CY}║${RS}  ${YE}Path    ${WH}: /  (หรือ /ssh/)                          ${CY}║${RS}\n"
  printf "${CY}╠══════════════════════════════════════════════════════╣${RS}\n"
  ...
  printf "${CY}║${RS}    ${GR}ws://%s:80/ → SSH${RS}\n" "$_h"
  printf "${CY}╠══════════════════════════════════════════════════════╣${RS}\n"
  printf "${CY}║${RS}  ${YE}UDPGW   ${WH}: 127.0.0.1:7300 (game/UDP)              ${CY}║${RS}\n"
  printf "${CY}║${RS}  ${YE}Auth    ${WH}: SSH user/pass ที่สร้างในข้อ 4           ${CY}║${RS}\n"
  printf "${CY}╚══════════════════════════════════════════════════════╝${RS}\n\n"
}
...
menu_18() {
  ...
  case $_sub18 in
    ...
    2)
      ...
      systemctl restart chaiya-sshws 2>/dev/null && printf "  ${GR}✅ chaiya-sshws${RS}\n" || printf "  ${RD}❌ chaiya-sshws${RS}\n"
