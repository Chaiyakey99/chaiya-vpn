...
for p in 22 80 81 109 143 443 2053 2082 7300 8080 8880; do
...
# หมายเหตุ: port 80 ถูกจัดการโดย chaiya-ws-dropbear (HTTP CONNECT tunnel)
...
# เพิ่ม -p 109 ถ้ายังไม่มี
grep -q '109' /etc/default/dropbear 2>/dev/null || \
  sed -i 's/DROPBEAR_EXTRA_ARGS=.*/DROPBEAR_EXTRA_ARGS="-p 109"/g' /etc/default/dropbear 2>/dev/null || true
...
# ── ws-stunnel Python (รับ HTTP CONNECT + payload → Dropbear) ─
cat > /usr/local/bin/chaiya-ws-dropbear << 'WSPYEOF'
#!/usr/bin/env python3
"""
Chaiya WS-Dropbear — HTTP CONNECT/GET payload tunnel
ฟัง port 80, รับ HTTP CONNECT หรือ GET payload จาก client
แล้ว forward ไป Dropbear port 143
รองรับทุกแอพ: NetMod, HTTP Injector, NapsternetV, KPN Tunnel ฯลฯ
"""
...
def handle(client, addr):
    remote = None
    try:
        # อ่าน HTTP request header จาก client
        buf = b""
        while b"\r\n\r\n" not in buf:
        ...
        # เชื่อมต่อไป Dropbear
        remote = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        remote.settimeout(15)
        remote.connect((TARGET_HOST, TARGET_PORT))
        remote.settimeout(None)

        # ตอบ HTTP 200 OK ให้ client ทุกกรณี (CONNECT, GET, PATCH ฯลฯ)
        client.sendall(b"HTTP/1.0 200 Connection established\r\n\r\n")

        log(f"CONNECT {addr[0]}:{addr[1]} -> Dropbear:{TARGET_PORT} [{first_line[:60]}]")

        # เริ่ม bidirectional forwarding
...
def main():
    log(f"Chaiya WS-Dropbear starting on :{LISTEN_PORT} -> Dropbear:{TARGET_PORT}")
...
if __name__ == "__main__":
    main()
WSPYEOF
chmod +x /usr/local/bin/chaiya-ws-dropbear

# ── chaiya-sshws systemd (ใช้ ws-dropbear Python แทน websocat) ─
...
[Service]
Type=simple
ExecStart=/usr/bin/python3 /usr/local/bin/chaiya-ws-dropbear
Restart=always
RestartSec=3
...
menu_2() {
  ...
  printf "${CY}│${RS}  🌐 โดเมน  : ${YE}%-39s${CY}│${RS}\n" "$_cur_domain"
  printf "${CY}│${RS}  🔌 WS Port: ${YE}%-39s${CY}│${RS}\n" "$_cur_wsport"
  printf "${CY}└────────────────────────────────────────────────────────┘${RS}\n\n"

  read -rp "$(printf "${YE}กรอกโดเมน (เช่น vpn.example.com): ${RS}")" domain
  [[ -z "$domain" ]] && { printf "${YE}↩ ยกเลิก${RS}\n"; read -rp "Enter..."; return; }

  printf "\n${YE}┌─[ เลือก Port สำหรับ WebSocket SSH tunnel ]────────────┐${RS}\n"
  printf "${YE}│${RS}  ${OR}⚠️  ห้ามใช้: 80 81 109 143 443 2053 8080 8880${RS}      ${YE}│${RS}\n"
  printf "${YE}├────────────────────────────────────────────────────────┤${RS}\n"
  ...
  # ── [1/4] หยุด services ที่ใช้ port 80 ──────────────────
  printf "\n${YE}⏳ [1/4] หยุด services บน port 80 ชั่วคราว...${RS}\n"
  local _stopped_svcs=()
  systemctl is-active --quiet nginx        && { systemctl stop nginx;        _stopped_svcs+=(nginx);        printf "  ${OR}⏹ nginx${RS}\n"; }
  systemctl is-active --quiet chaiya-sshws && { systemctl stop chaiya-sshws; _stopped_svcs+=(chaiya-sshws); printf "  ${OR}⏹ chaiya-sshws${RS}\n"; }
  local _w=0
  while ss -tlnp 2>/dev/null | grep -q ':80 ' && (( _w < 10 )); do sleep 1; (( _w++ )) || true; done
  if ss -tlnp 2>/dev/null | grep -q ':80 '; then
    printf "${RD}❌ port 80 ยังถูกใช้อยู่ — หยุด service ที่ใช้ port 80 ด้วยตนเองก่อน${RS}\n"
    for _s in "${_stopped_svcs[@]}"; do systemctl start "$_s" 2>/dev/null || true; done
...
upstream chaiya_sshws_backend {
    server unix:/var/run/chaiya-sshws.sock;
}
server {
...
server {
    ...
    proxy_buffering off;
    location / { proxy_pass http://chaiya_sshws_backend; }
}
SSLEOF
    ...
    nginx -t 2>/dev/null && printf "  ${GR}✅ nginx config OK${RS}\n" || { printf "  ${RD}❌ nginx config error${RS}\n"; nginx -t; }

    # cron auto-renew: หยุด → renew → เริ่มใหม่
    (crontab -l 2>/dev/null || true) | grep -v certbot-renew | \
      { cat; echo "0 3 * * * systemctl stop chaiya-sshws nginx; certbot renew --quiet --standalone; systemctl start nginx chaiya-sshws # certbot-renew"; } | crontab -
  ...
  # ── [4/4] เริ่ม services กลับอัตโนมัติ ──────────────────
  printf "\n${YE}⏳ [4/4] เริ่ม services กลับ...${RS}\n"
  systemctl start nginx        2>/dev/null && printf "  ${GR}✅ nginx${RS}\n"        || printf "  ${RD}❌ nginx${RS}\n"
  systemctl start chaiya-sshws 2>/dev/null && printf "  ${GR}✅ chaiya-sshws${RS}\n" || printf "  ${RD}❌ chaiya-sshws${RS}\n"
...
menu_16() {
  ...
  if systemctl is-active dropbear &>/dev/null; then
    ...
    printf "${CY}│${RS}  ${GR}✅ ติดตั้ง Dropbear สำเร็จ (port 143/109)${RS}\n"
  fi
  ...
  printf "${CY}│${RS}  ${YE}⏳ ตรวจสอบ chaiya-ws-dropbear...${RS}\n"
...
_m18_setup_nginx_ws() {
  ...
  if $_cert_ok; then
    ...
    # ── กรณีไม่มี SSL: สร้าง plain WS block ─────────────────
    cat > /etc/nginx/sites-available/chaiya-sshws << NGINXWS
# ── upstream: websocat UNIX socket
upstream chaiya_sshws_backend {
    server unix:/var/run/chaiya-sshws.sock;
}

# ── Port ${_wsport}: WebSocket SSH tunnel → SSH port 22
server {
    listen ${_wsport} default_server;
    server_name _;
    ...
    location / {
        proxy_pass http://chaiya_sshws_backend;
    }
}
NGINXWS
...
# ── helper: สร้าง systemd service chaiya-sshws (websocat) ───
_m18_setup_systemd() {
  # ── ลบ socket เก่าก่อน (ป้องกัน "address already in use") ──
  rm -f /var/run/chaiya-sshws.sock 2>/dev/null || true

  # ── wrapper script: chmod socket หลัง websocat สร้างเสร็จ ──
  cat > /usr/local/bin/chaiya-sshws-start << 'WRAPEOF'
#!/bin/bash
# ลบ socket เก่า
rm -f /var/run/chaiya-sshws.sock 2>/dev/null || true

# เริ่ม websocat แบบ background ก่อน แล้ว chmod socket
/usr/local/bin/websocat \
  --binary \
  -s unix-l:/var/run/chaiya-sshws.sock \
  tcp:127.0.0.1:22 &
WS_PID=$!

# รอ socket ถูกสร้าง (max 5 วินาที)
for i in $(seq 1 50); do
  if [[ -S /var/run/chaiya-sshws.sock ]]; then
    chmod 777 /var/run/chaiya-sshws.sock
    break
  fi
  sleep 0.1
done

# wait ให้ process หลักทำงานต่อ (foreground สำหรับ systemd)
wait $WS_PID
WRAPEOF
  chmod +x /usr/local/bin/chaiya-sshws-start

  cat > /etc/systemd/system/chaiya-sshws.service << 'SVCEOF'
[Unit]
Description=Chaiya SSH WebSocket Tunnel (websocat unix-socket -> SSH :22)
After=network.target ssh.service
Wants=network.target

[Service]
Type=simple
ExecStart=/usr/local/bin/chaiya-sshws-start
Restart=always
RestartSec=5
StandardOutput=append:/var/log/chaiya-sshws.log
StandardError=append:/var/log/chaiya-sshws.log
...
_m18_status() {
  ...
  local SOCK_OK="❌ ไม่มี socket"
  [[ -S /var/run/chaiya-sshws.sock ]] && SOCK_OK="✅ socket พร้อม"
  local WS_CMD=$(command -v websocat &>/dev/null && echo "✅ $(websocat --version 2>/dev/null | head -1)" || echo "❌ ยังไม่ติดตั้ง")
...
menu_18() {
  ...
  printf "\n${R2}┌─[ 🔧 เมนู SSH WebSocket Manager ]─────────────────────┐${RS}\n"
  printf "${R2}│${RS}  ${GR}1.${RS}  🔧 ติดตั้ง websocat + nginx WS block + systemd   ${R2}│${RS}\n"
  printf "${R2}│${RS}  ${CY}2.${RS}  🔄 Restart ทุก service (websocat/nginx/udpgw)   ${R2}│${RS}\n"
  printf "${R2}│${RS}  ${RD}3.${RS}  ⏹  หยุด chaiya-sshws (WebSocket tunnel)         ${R2}│${RS}\n"
  printf "${R2}│${RS}  ${GR}4.${RS}  👤 สร้าง SSH User (expire + /bin/false)         ${R2}│${RS}\n"
  printf "${R2}│${RS}  ${YE}5.${RS}  👥 ดูรายชื่อ SSH Users ทั้งหมด                   ${R2}│${RS}\n"
  printf "${R2}│${RS}  ${RD}6.${RS}  🗑️  ลบ SSH User                                  ${R2}│${RS}\n"
  printf "${R2}│${RS}  ${CY}7.${RS}  📱 แสดง Config สำหรับแอพ (payload/host/port)   ${R2}│${RS}\n"
  printf "${R2}│${RS}  ${OR}8.${RS}  📋 ดู Log chaiya-sshws (30 บรรทัดล่าสุด)        ${R2}│${RS}\n"
  printf "${R2}│${RS}  ${GR}9.${RS}  🎮 Restart badvpn-udpgw (port 7300)             ${R2}│${RS}\n"
  printf "${R2}│${RS}  ${WH}0.${RS}  ↩  ย้อนกลับ                                      ${R2}│${RS}\n"
  printf "${R2}└────────────────────────────────────────────────────────┘${RS}\n"
  read -rp "$(printf "\n${YE}เลือก: ${RS}")" _sub18
  ...
  case $_sub18 in
    1)
      clear
      printf "${OR}╔══════════════════════════════════════════════════════╗${RS}\n"
      printf "${OR}║${RS}  🔧 ${WH}ติดตั้ง SSH WebSocket Stack${RS}                     ${OR}║${RS}\n"
      printf "${OR}╚══════════════════════════════════════════════════════╝${RS}\n\n"

      # 1) websocat binary
      printf "${YE}[1/3]${RS} ติดตั้ง websocat...\n"
      _m18_install_websocat || { read -rp "$(printf "${RD}Enter ย้อนกลับ...${RS}")"; menu_18; return; }

      # 2) nginx WebSocket block
      printf "\n${YE}[2/3]${RS} ตั้งค่า nginx WebSocket block port 80...\n"
      _m18_setup_nginx_ws

      # 3) systemd service
      printf "\n${YE}[3/3]${RS} สร้าง systemd service chaiya-sshws...\n"
      _m18_setup_systemd

      printf "\n${GR}╔══════════════════════════════════════════════════════╗${RS}\n"
      printf "${GR}║${RS}  ✅ ${WH}ติดตั้งครบแล้ว! สรุปการทำงาน:${RS}                ${GR}║${RS}\n"
      printf "${GR}║${RS}  ${WH}Client → nginx:80 (WS) → UNIX socket${RS}             ${GR}║${RS}\n"
      printf "${GR}║${RS}  ${WH}         → websocat → SSH port 22${RS}                 ${GR}║${RS}\n"
      printf "${GR}║${RS}  ${YE}สร้าง user ได้ที่เมนู 4${RS}                          ${GR}║${RS}\n"
      printf "${GR}╚══════════════════════════════════════════════════════╝${RS}\n\n"
      read -rp "$(printf "${YE}Enter ย้อนกลับ...${RS}")"
      menu_18 ;;
    ...
    2)
      ...
      # ลบ socket เก่าก่อน restart เพื่อป้องกัน "address in use"
      rm -f /var/run/chaiya-sshws.sock 2>/dev/null || true
      systemctl restart chaiya-sshws 2>/dev/null
      # รอ socket ปรากฏ (max 8 วินาที) แล้วค่อย reload nginx
      local _w=0
      printf "  ${YE}⏳ รอ socket...${RS}"
      while [[ ! -S /var/run/chaiya-sshws.sock ]] && (( _w < 8 )); do
        sleep 1; (( _w++ )) || true; printf "."
      done; echo ""
      if [[ -S /var/run/chaiya-sshws.sock ]]; then
        printf "  ${GR}✅ chaiya-sshws + socket พร้อม${RS}\n"
      else
        printf "  ${RD}❌ chaiya-sshws หรือ socket ยังไม่พร้อม${RS}\n"
        journalctl -u chaiya-sshws -n 5 --no-pager 2>/dev/null | tail -5
      fi
      systemctl reload nginx 2>/dev/null        && printf "  ${GR}✅ nginx${RS}\n"    || printf "  ${RD}❌ nginx${RS}\n"
      systemctl restart dropbear 2>/dev/null    && printf "  ${GR}✅ dropbear${RS}\n" || printf "  ${YE}⚠️  dropbear (ข้ามได้ถ้าใช้ OpenSSH)${RS}\n"
    ...
    4)
      clear
      printf "${GR}╔══════════════════════════════════════════════════════╗${RS}\n"
      printf "${GR}║${RS}  👤 ${WH}สร้าง SSH User (WebSocket Tunnel)${RS}              ${GR}║${RS}\n"
      printf "${GR}╚══════════════════════════════════════════════════════╝${RS}\n\n"
      _m18_add_ssh_user
      read -rp "$(printf "\n${YE}Enter ย้อนกลับ...${RS}")"
      menu_18 ;;
    ...
    8)
      printf "\n${CY}┌─[ 📋 Log chaiya-sshws (30 บรรทัดล่าสุด) ]──────────────┐${RS}\n"
      local _log="/var/log/chaiya-sshws.log"
      if [[ -f "$_log" && -s "$_log" ]]; then
        tail -30 "$_log" | while IFS= read -r line; do
          printf "${CY}│${RS}  ${WH}%.60s${RS}\n" "$line"
        done
        printf "${CY}├─────────────────────────────────────────────────────────┤${RS}\n"
        printf "${CY}│${RS}  ${YE}Log path: %s${RS}\n" "$_log"
      else
        printf "${CY}│${RS}  ${OR}ยังไม่มี log${RS}\n"
      fi
      printf "${CY}└─────────────────────────────────────────────────────────┘${RS}\n\n"
      # journalctl เสริม
      printf "${CY}┌─[ journalctl -u chaiya-sshws -n 10 ]───────────────────┐${RS}\n"
      journalctl -u chaiya-sshws -n 10 --no-pager 2>/dev/null | while IFS= read -r line; do
        printf "${CY}│${RS}  ${WH}%.60s${RS}\n" "$line"
      done
      printf "${CY}└─────────────────────────────────────────────────────────┘${RS}\n\n"
      read -rp "$(printf "${YE}Enter ย้อนกลับ...${RS}")"
