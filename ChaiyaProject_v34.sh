  ...
  if (!users.error && users.users) {
    dl.innerHTML = users.users.map(u => {
      ...
      return `<div style="margin-bottom:.8rem">
        ...
        <div class="progress">
          <div class="progress-bar" style="width:${limit>0?pct:100}%;background:${limit>0?`linear-gradient(90deg,${bc},${bc}88)`:'linear-gradient(90deg,#00ff80,#00d4ff,#b400ff)'};box-shadow:0 0 6px ${bc}55;transition:width .8s ease"></div>
        </div>
        ${limit>0?`<div style="text-align:right;font-size:.7rem;color:${bc}88;margin-top:2px">${pct.toFixed(1)}%</div>`:''}
      </div>`;
    }).join('');
  }
}
...
LICENSE_SERVER="http://license.godvpn.shop:7070"   # ← เปลี่ยนเป็น URL server คุณ
...
" 2>/dev/null || echo "999")
  ...
  printf "${RD}╚══════════════════════════════════════════════════╝${RS}\n\n"
  # ลบ key ที่บันทึกไว้ถ้า invalid
  [[ "$msg" == "invalid_key" || "$msg" == "key_disabled" ]] && rm -f "$LICENSE_FILE" 2>/dev/null
  exit 1
}
...
.bar-fill{height:100%;border-radius:99px;
          background:linear-gradient(90deg,""" + bc + """,""" + bc + """88);
          animation:barFill 1.2s cubic-bezier(.4,0,.2,1) forwards;
          box-shadow:0 0 8px """ + bc + """66}
...
async function fetchTraffic(){
  ...
  try{
    ...
    var limGb   = d.limit_gb || DATA_LIMIT_GB || 0;
    var pct     = d.pct      || 0;
    ...
    if(limGb > 0){
      ...
      barEl.style.background = 'linear-gradient(90deg,#00ff80,#00d4ff,#b400ff)';
      barEl.style.animation  = 'none';
      barEl.style.boxShadow  = '0 0 8px #00d4ff44';
      pctEl.textContent      = '\\u221e \\u0e44\\u0e21\\u0e48\\u0e08\\u0e33\\u0e01\\u0e31\\u0e14';
...
BASHRC_BLOCK='
# ── CHAIYA auto-launch ────────────────────────────────────────
if [[ $- == *i* ]] && [[ "$(id -u)" == "0" ]] && [[ -x /usr/local/bin/chaiya ]]; then
  exec /usr/local/bin/chaiya
fi'

# เขียนลง /root/.bashrc (ถ้ายังไม่มี)
if ! grep -q "CHAIYA auto-launch" /root/.bashrc 2>/dev/null; then
  echo "$BASHRC_BLOCK" >> /root/.bashrc
  echo "✅ ตั้ง auto-launch เมนู chaiya เมื่อ root login สำเร็จ"
fi
...
echo "║                                                          ║"
echo "║  🔄 เมนูจะเปิดอัตโนมัติทุกครั้งที่ login root          ║"
echo "╰══════════════════════════════════════════════════════════╯"
echo ""

# เปิดเมนู chaiya ทันทีหลังติดตั้งเสร็จ
exec /usr/local/bin/chaiya
