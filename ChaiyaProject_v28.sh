#!/bin/bash
# ============================================================
#  CHAIYA V2RAY PRO MAX v10 (script v28)  —  Full Auto Setup
...
menu_2() {
  ...
  if $_cert_ok; then
    ...
    # PORT 80 สงวนไว้สำหรับ chaiya-ws-dropbear เท่านั้น — nginx ห้าม listen 80
    cat > /etc/nginx/sites-available/chaiya-ssl << SSLEOF
# port 81: dashboard (ห้าม listen 80)
server {
    listen 81;
    server_name _;
    root /var/www/chaiya;
...
_m18_status() {
  ...
  elif pgrep -f "chaiya-ws-dropbear" &>/dev/null; then
