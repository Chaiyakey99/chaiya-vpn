...
"
  ...
  rgb_bar 10 "ดาวน์โหลด install script..."; printf "\n"
  ...
  # ── 15–45% ติดตั้ง 3x-ui ──
  rgb_bar 15 "กำลังติดตั้ง 3x-ui..."; printf "\n"
  local _pct_install=15
  printf "y\n2053\n2\n\n80\n" | bash "$_xui_sh" >> /var/log/chaiya-xui-install.log 2>&1 &
  local _install_pid=$!
  while kill -0 "$_install_pid" 2>/dev/null; do
    (( _pct_install < 45 )) && (( _pct_install++ ))
    rgb_bar "$_pct_install" "กำลังติดตั้ง 3x-ui..."
    sleep 1
  done
  ...
  # ── 50% reset credential ──
  rgb_bar 50 "ตั้งค่า credential..."; printf "\n"
  ...
  # ── 55–75% รอ port ──
  rgb_bar 55 "รอ port ${_panel_port}..."; printf "\n"
  local _ok=0
  for _i in $(seq 1 10); do
    local _pp2=$(( 55 + _i * 2 ))
    rgb_bar "$_pp2" "รอ port ${_panel_port} (${_i}/10)..."
  ...
  for _item in "${_inbounds[@]}"; do
    ...
    rgb_bar $(( 85 + _ib_n * 5 )) "สร้าง inbound port ${_ibport}..."; printf "\n"
  ...
  "sniffing":{"enabled":true,"destOverride":["http","tls"]}
    ...
    ufw allow "${_ibport}"/tcp 2>/dev/null || true
  ...
  printf "${R1}┌──────────────────────────────────────────────────┐${RS}\n"
  printf "${R1}│${RS}  🌐 Panel  : ${WH}http://%s:%s${RS}\n" "$MY_IP" "$_panel_port"
  printf "${R1}│${RS}  🔗 API    : ${WH}http://%s:%s%s/panel/api${RS}\n" "$MY_IP" "$_panel_port" "$_bp_clean"
